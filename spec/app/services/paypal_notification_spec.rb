      ...
      context "when an exception is raised" do
        ...
        it "marks the donation as failed, with a reason" do
          ...
          donation.should_receive(:fail_reason=) do |reason|
            reason[:message].should_not be_blank
            reason[:backtrace].should_not be_blank
          end
          subject.process!
        end
  ...
  it "works in integration" do
    ...
    Timecop.freeze(Date.new(2012, 3, 2)) do
      VCR.use_cassette('donation') do
        ...
        donation.reload
        donation.transaction_id.should == "53Y12314WR4111134"
        donation.status.should == Donation::STATUS_DONE
      end
    end
  end
