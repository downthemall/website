...
describe Persona do
  ...
  describe ".verify_assertion!" do
    it "raises InvalidResponse if status is not okay" do
      subject.stubs(:request_assertion).with('XXX').returns(OpenStruct.new(status: "fail"))
    ...
    it "raises InvalidResponse if audience returned does not match" do
      subject.stubs(:request_assertion).with('XXX').returns(OpenStruct.new(status: "okay", audience: 'foobar.com'))
      expect(-> { subject.verify_assertion!('XXX') }).to raise_error(Persona::InvalidResponse)
    end
    it "raises InvalidResponse if already expired" do
      Timecop.freeze(Time.new(2013, 3, 24)) do
        subject.stubs(:request_assertion).with('XXX').returns(OpenStruct.new(status: "okay", audience: 'downthemall.com', expires: 1364027501 ))
        expect(-> { subject.verify_assertion!('XXX') }).to raise_error(Persona::InvalidResponse)
      end
    end
